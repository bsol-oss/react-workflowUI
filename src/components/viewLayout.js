...
const recurseWorkflow = async (nodes, taskId) => {
  ...
  let nextApprovedTaskIds =
    task.data.NextAction.APPROVED.length > 0
      ? task.data.NextAction.APPROVED
      : [];
  let nextREJECTEDTaskIds =
    task.data.NextAction.REJECTED.length > 0
      ? task.data.NextAction.REJECTED
      : [];
...
const ViewLayoutFlow = (workflowJson) => {
  ...
  const onLayout = useCallback(
    ({ direction }) => {
      ...
      const { nodes: tempNodes1, edges: tempEdges1 } = workflowToNodesEdges(
      ...
      <div style={{ width: "100%", height: "100%" }} id="rootprovider">
        ...
        <ReactFlowProvider
          style={{
            width: "100%",
            height: "100%",
            fontFamily: "roboto, Noto Sans TC, Noto Sans SC, sans-serif",
          }}
        >
          <div
